...
<div class=" mx-auto bg-white p-8 rounded-lg shadow-md">
    <h2 class="text-3xl font-bold mb-8 text-gray-800">Create New Task</h2>
    ...
    <form method="POST" action="{{ route('task.store') }}">
        ...
        <div class="mb-6">
            ...
            <input class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('title') border-red-500 @enderror" type="text" id="title" name="title" placeholder="Enter task title" value="{{ old('title') }}" required>
            @error('title')
        ...
        <div class="mb-6">
            ...
            <textarea class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror" id="description" name="description" placeholder="Enter task description" rows="4">{{ old('description') }}</textarea>
            @error('description')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('user_id') border-red-500 @enderror" id="user_id" name="user_id" required>
                ...
                <option value="{{ $employee->id }}" {{ old('user_id') == $employee->id ? 'selected' : '' }}>{{ $employee->name }} ({{ $employee->email }})</option>
                @empty
        ...
        <div class="mb-6">
            ...
            <input class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('due_at') border-red-500 @enderror" type="datetime-local" id="due_at" name="due_at" value="{{ old('due_at') }}" required>
            @error('due_at')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="flex gap-4">
            <button class="flex-1 bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600 transition duration-200 font-semibold" type="submit">Create Task</button>
